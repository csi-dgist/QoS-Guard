...
CycloneDDS QoS policy implementation status:
- O (Implemented): RELIABILITY, DURABILITY, DEADLINE, LIVELINESS, LEASE_DURATION, 
                   HISTORY, DEPTH, RESOURCE_LIMITS, LIFESPAN
- X (Not Implemented): ENTITY_FACTORY, PARTITION, USER_DATA, GROUP_DATA, TOPIC_DATA,
                       OWNERSHIP, OWNERSHIP_STRENGTH, DESTINATION_ORDER,
...
def rule_durable_partition(_xml, q, _ctx):
    """[X] Not Implemented: PARTITION."""
    # Only show Not Implemented when PARTITION is actually set
    if _has_non_empty_partition(q):
        return "Not Implemented: PARTITION"
    return None
...
def rule_autodispose_with_exclusive(_xml, q, _ctx):
    """[X] Not Implemented: WRITER_DATA_LIFECYCLE or OWNERSHIP."""
    # Only show Not Implemented when WRITER_DATA_LIFECYCLE or OWNERSHIP is actually set
    if (q.get("autodispose_unregistered_instances", "").strip() or 
        q.get("ownership", "").strip()):
        return "Not Implemented: WRITER_DATA_LIFECYCLE or OWNERSHIP"
    return None
...
def rule_exclusive_lease_infinite(xml, q, _ctx):
    """[X] Not Implemented: OWNERSHIP."""
    # Only show Not Implemented when OWNERSHIP is actually set
    if q.get("ownership", "").strip():
        return "Not Implemented: OWNERSHIP"
    return None
...
def rule_partition_overlap(pub_xml, sub_xml):
    ...
    if w_parts or r_parts:  # If any partition is configured
        return "Not Implemented: PARTITION"
    return None
...
CROSS_RULES_XML: List[Tuple[Callable, str]] = [
    (rule_partition_overlap, "Structural"),  # 18: PART↔PART (X)
...
CROSS_RULES_PROFILES: List[Tuple[Callable, str]] = [
    ...
    (rule_ownership_compat, "Structural"),  # 23: OWNST↔OWNST (X)
