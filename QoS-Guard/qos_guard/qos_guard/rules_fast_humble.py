#!/usr/bin/env python3
"""
Basic QoS rule checking module for Humble Fast DDS 2.6.9.

Humble Fast DDS 2.6.9 QoS policy implementation status:
- O (Implemented): PARTITION, USER_DATA, GROUP_DATA, TOPIC_DATA, RELIABILITY, DURABILITY, 
                   DEADLINE, LIVELINESS, LEASE_DURATION, HISTORY, DEPTH, LIFESPAN
- △ (Partially Implemented): ENTITY_FACTORY, OWNERSHIP, OWNERSHIP_STRENGTH
- X (Not Implemented): DESTINATION_ORDER, WRITER_DATA_LIFECYCLE, READER_DATA_LIFECYCLE

For X policies, "Not Implemented" is shown only when the policy is actually configured.
For △ policies, "Partially Implemented" is shown only when the policy is actually configured.
"""
import math
import re
...
def rule_autoenable_vs_volatile_reader(_xml, q, _ctx):
    """[△] Partially Implemented: ENTITY_FACTORY."""
    # Only show Partially Implemented when ENTITY_FACTORY is actually set
    if q.get("autoenable", "").strip():
        return "Partially Implemented: ENTITY_FACTORY"
    return None
...
def rule_autodispose_with_exclusive(_xml, q, _ctx):
    """[X] Not Implemented: WRITER_DATA_LIFECYCLE or [△] Partially Implemented: OWNERSHIP."""
    # Check for WRITER_DATA_LIFECYCLE first (X)
    if q.get("autodispose_unregistered_instances", "").strip():
        return "Not Implemented: WRITER_DATA_LIFECYCLE"
    # Then check for OWNERSHIP (△)
    if q.get("ownership", "").strip():
        return "Partially Implemented: OWNERSHIP"
    return None
...
def rule_exclusive_lease_infinite(xml, q, _ctx):
    """[△] Partially Implemented: OWNERSHIP."""
    # Only show Partially Implemented when OWNERSHIP is actually set
    if q.get("ownership", "").strip():
        return "Partially Implemented: OWNERSHIP"
    return None
...
def rule_ownership_compat(pub_q, sub_q):
    """[△] Partially Implemented: OWNERSHIP."""
    # Only show Partially Implemented when OWNERSHIP is actually set
    if (pub_q.get("ownership", "").strip() or 
        sub_q.get("ownership", "").strip()):
        return "Partially Implemented: OWNERSHIP"
    return None
