#!/usr/bin/env python3
"""
Basic QoS rule checking module for Jazzy Fast DDS 2.14.0.

Jazzy Fast DDS 2.14.0 QoS policy implementation status:
...
def _autopurge_disposed_gt_zero(q: dict) -> bool:
    """Check RDLIFE.autopurge_disposed_samples_delay is FINITE and > 0 (≠ ∞).
    ∞ (DURATION_INFINITY) is the DDS default and must NOT trigger Rule 29 (2026-07-13)."""
    val = (q.get("autopurge_disposed_samples_delay") or "").strip()
    if not val:
        return False
    if val.upper() in INF_SET:
        return False  # ∞ (unlimited) = DDS default → Rule 29 fires only for FINITE non-default delay
    try:
        return int(val) > 0
    except ValueError:
        return False
