...
# Conservative default publish period. PP >= 2*RTT keeps the buffer threshold
# ceil(2*RTT/PP)+1 at 2, so a default run flags only depth-1. Pass --auto-pp to
# derive PP from the source.
DEFAULT_PUBLISH_PERIOD_MS = 100
DEFAULT_RTT_MS = 50

USAGE = f"""Usage:
  Package mode (default):
    qos_guard <package_path> [dds] [ros_version] [--auto-pp] [publish_period=<Nms>] [rtt=<Nms>]
  ...
  Publish period (package mode):
    default   {DEFAULT_PUBLISH_PERIOD_MS}ms conservative lower bound (threshold flags only depth-1 at the default RTT)
    --auto-pp auto-detect each topic's period from the timer that drives it, using {DEFAULT_PUBLISH_PERIOD_MS}ms where none is found
    publish_period=<Nms> pins one explicit period for every topic and overrides --auto-pp

Examples:
  qos_guard /path/to/ros2_package                    # Package mode, conservative default PP
  qos_guard /path/to/ros2_package fast jazzy        # Package mode with specific DDS/ROS
  qos_guard /path/to/ros2_package cyclone jazzy --auto-pp   # Auto-detect each topic's PP
...
class XmlPairArgs:
    ...
    rtt_ns: int
    publish_period_explicit: bool = False
...
class PackageArgs:
    ...
    # True only when the user passed publish_period=<Nms> on the CLI (pins PP).
    publish_period_explicit: bool = False
    # True when --auto-pp derives PP from the package source. Ignored when
    # publish_period_explicit is set.
    auto_pp: bool = False
...
def parse_args(argv: list[str]) -> CliArgs | ListXmlArgs:
    ...
    if len(argv) >= 2 and argv[1].strip().lower() in ("--xml", "-x"):
        ...
        return XmlPairArgs(
            ...
            rtt_ns=rtt_ns,
            publish_period_explicit=publish_period_explicit,
        )
    ...
    while i < len(argv):
        ...
        if arg in DDS_CHOICES:
            ...
            ros_version = arg
        elif arg.lower() in ("--auto-pp", "auto-pp"):
            auto_pp = True
        elif _is_kv_arg(arg):
            try:
                if arg.startswith("publish_period="):
