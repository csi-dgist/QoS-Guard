#!/usr/bin/env python3
"""
Basic QoS rule checking module for CycloneDDS.

CycloneDDS QoS policy implementation status (Cyclone DDS 0.10.5):
- O (Implemented): RELIABILITY, DURABILITY, DEADLINE, LIVELINESS, LEASE_DURATION,
                   HISTORY, DEPTH, RESOURCE_LIMITS, LIFESPAN,
                   OWNERSHIP, OWNERSHIP_STRENGTH, DESTINATION_ORDER, PARTITION
- X (Not Implemented): ENTITY_FACTORY, USER_DATA, GROUP_DATA, TOPIC_DATA,
                       WRITER_DATA_LIFECYCLE, READER_DATA_LIFECYCLE

WRITER_DATA_LIFECYCLE is kept X pending review. Rule 16 (autodispose + EXCLUSIVE)
runs as an ownership-arbitration rule. For remaining X policies, "Not Implemented"
is shown only when the policy is actually configured.
"""
import math
import re
...
def deadline_enabled(xml):
    """DEADLINE is 'enabled' only for a FINITE, non-zero period.
    An unset or infinite (∞ = DURATION_INFINITY) deadline is the DDS default and is
    treated as NOT enabled, so rules gating on it (14/35/40/38) do not fire on ∞."""
    period_ns = xp.deadline_period_ns(xml)  # None if unset or ∞
    return period_ns is not None and period_ns != 0
...
def rule_autoenable_vs_volatile_reader(_xml, q, _ctx):
    """[X] Not Implemented: ENTITY_FACTORY."""
    # Only show Not Implemented when ENTITY_FACTORY is actually set
    if q.get("autoenable", "").strip():
        return "Not Implemented: ENTITY_FACTORY"
    return None
...
def rule_autodispose_with_exclusive(_xml, q, _ctx):
    """Warn autodispose with EXCLUSIVE ownership. (Rule 16: OWNST→WDLIFE, ownership arbitration.)"""
...
def rule_stage3_autodispose_autopurge_cross(pub_q: dict, sub_q: dict):
    """[X] Not Implemented: WRITER_DATA_LIFECYCLE or READER_DATA_LIFECYCLE."""
    # Only show Not Implemented when LIFECYCLE policies are actually set
    if (pub_q.get("autodispose_unregistered_instances", "").strip() or 
        pub_q.get("autopurge_nowriter_samples", "").strip() or 
        pub_q.get("autopurge_disposed_samples_delay", "").strip() or
        sub_q.get("autopurge_nowriter_samples", "").strip() or 
        sub_q.get("autopurge_disposed_samples_delay", "").strip()):
        return "Not Implemented: WRITER_DATA_LIFECYCLE or READER_DATA_LIFECYCLE"
    return None
...
def rule_stage3_reliable_keep_last_depth(xml, q, ctx):
    """[RELIAB=RELIABLE] ∧ [HIST=KEEP_LAST] ∧ [depth < ⌈2·RTT/PP⌉+1] - Functional."""
    ...
    depth = int(depth_txt)
    if depth <= 1:
        return "Invalid QoS: RELIAB=RELIABLE + KEEP_LAST(depth=1) <-> reliable buffer needs >=2 for any RTT/PP"
    pp_sec = ctx.publish_period_ms / 1000
    rtt_sec = ctx.rtt_ns / 1_000_000_000
    required = math.ceil(2 * rtt_sec / pp_sec) + 1
    if depth >= required:
        return None
    return f"Invalid QoS: RELIAB=RELIABLE + KEEP_LAST(depth={depth}) <-> ⌈2·RTT/PP⌉+1({required})"


def rule_stage3_reliable_keepall_mpi(xml, q, ctx):
    """[RELIAB=RELIABLE] ∧ [HIST=KEEP_ALL] ∧ [mpi < ⌈2·RTT/PP⌉+1] - Functional."""
    if q.get("reliability", "").strip().upper() != "RELIABLE":
        return None
    if q.get("history", "").strip().upper() != "KEEP_ALL":
        return None
    ...
    mpi = int(mpi_txt)
    if mpi <= 1:
        return "Invalid QoS: RELIAB=RELIABLE + KEEP_ALL(mpi=1) <-> reliable buffer needs >=2 for any RTT/PP"
    pp_sec = ctx.publish_period_ms / 1000
    rtt_sec = ctx.rtt_ns / 1_000_000_000
    required = math.ceil(2 * rtt_sec / pp_sec) + 1
    if mpi >= required:
        return None
    return f"Invalid QoS: RELIAB=RELIABLE + KEEP_ALL + mpi({mpi}) <-> ⌈2·RTT/PP⌉+1({required})"


def rule_stage3_reliable_lifespan(xml, q, ctx):
    """[RELIAB=RELIABLE] ∧ [LFSPAN.duration < PP+2·RTT] - Functional (Table 31)."""
    if q.get("reliability", "").strip().upper() != "RELIABLE":
        return None
    ...
    lifespan_ns = ls_sec * 1_000_000_000 + ls_nsec
    pp_ns = ctx.publish_period_ms * 1_000_000
    if lifespan_ns >= pp_ns + 2 * ctx.rtt_ns:
        return None
    return "Invalid QoS: RELIAB=RELIABLE + LFSPAN <-> PP+2·RTT"
...
def rule_stage3_lease_deadline(xml, _q, ctx):
    ...
    if not dl_m or not ld_m:
        return None
    if deadline_period_ns(xml) is None:  # DEADLN.period ∞/unset = DDS default → no false positive
        return None
    dl_sec = parse_duration_field(dl_m.group(1))
    dl_nsec = parse_duration_field(dl_m.group(2))
...
def rule_stage3_exclusive_deadline_short(xml, q, ctx):
    """[OWNST.kind = EXCLUSIVE] ∧ [DEADLN.period < PP+2·RTT] - Operational."""
    if q.get("ownership", "").strip().upper() != "EXCLUSIVE":
        return None
    if not deadline_enabled(xml):
        return None
    ...
    if dl_ns >= pub_ns + 2 * ctx.rtt_ns:
        return None
    return f"Invalid QoS: OWNST=EXCLUSIVE + DEADLN.period <-> PP+2·RTT({(pub_ns + 2 * ctx.rtt_ns) // 1_000_000}ms)"


def rule_stage3_exclusive_lease_short(xml, q, ctx):
    """[OWNST.kind = EXCLUSIVE] ∧ [LIVENS.lease_duration < PP+2·RTT] - Operational."""
    if q.get("ownership", "").strip().upper() != "EXCLUSIVE":
        return None
    ...
    if lease_ns is None:
        return None
    pub_ns = ctx.publish_period_ms * 1_000_000
    if lease_ns >= pub_ns + 2 * ctx.rtt_ns:
        return None
    return f"Invalid QoS: OWNST=EXCLUSIVE + LIVENS.lease <-> PP+2·RTT({(pub_ns + 2 * ctx.rtt_ns) // 1_000_000}ms)"


def rule_stage3_autodispose_best_effort(_xml, q, _ctx):
    """[X] Not Implemented: WRITER_DATA_LIFECYCLE."""
    # Only show Not Implemented when WRITER_DATA_LIFECYCLE is actually set
    if q.get("autodispose_unregistered_instances", "").strip():
        return "Not Implemented: WRITER_DATA_LIFECYCLE"
    return None
...
ENTITY_RULES: List[Tuple[Callable, str, str]] = [
    ...
    # 17: OWNST→WDLIFE (Rule 16, ownership arbitration; WDLIFE label under review)
...
def rule_nowriter_autodispose_zero(pub_q, sub_q):
    """[X] Not Implemented: LIFECYCLE."""
    # Only show Not Implemented when LIFECYCLE policies are actually set
    if (pub_q.get("autodispose_unregistered_instances", "").strip() or
        sub_q.get("autopurge_nowriter_samples", "").strip() or 
        sub_q.get("autopurge_disposed_samples_delay", "").strip()):
        return "Not Implemented: LIFECYCLE"
    return None
