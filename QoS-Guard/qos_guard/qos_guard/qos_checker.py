...
def _apply_qos_defaults(qos: dict, dds: str = "fast") -> dict:
    ...
    # The RMW default reliability is reliable, so an unresolved endpoint is
    # reliable rather than best-effort. Leaving it empty would misread such an
    # endpoint as best-effort and raise a spurious RxO reliability conflict.
    if not (out.get("reliability") and str(out["reliability"]).strip()):
...
def main() -> None:
    ...
    # Publish-period resolution (package mode): explicit publish_period wins,
    # else --auto-pp derives each topic's period, else the conservative default.
    topic_periods: dict[str, int] = {}
    if args.mode == "package" and not getattr(args, "publish_period_explicit", False):
        if getattr(args, "auto_pp", False):
            topic_periods = pp_extractor.extract_topic_periods(args.package_path)
            output.print_info(
                f"Auto-detected per-topic publish periods for {len(topic_periods)} "
                f"topic(s); using the default {args.publish_period_ms}ms where no "
                "timer drives the topic"
            )
        else:
            output.print_info(
                f"Using conservative publish period {args.publish_period_ms}ms "
                "(pass --auto-pp to auto-detect each topic's period, or "
                "publish_period=<Nms> to pin it)"
            )
    ...
    # Per-topic publish period: with --auto-pp each topic uses its own timer's
    # period (topic_periods); everything else uses the default/explicit value.
    default_pp = args.publish_period_ms
    _norm_periods = {t.strip().strip("/"): v for t, v in topic_periods.items()}

    def _pp_for(topic: str) -> int:
        return _norm_periods.get((topic or "").strip().strip("/"), default_pp)
    ...
    if args.mode == "xml_pair":
        ...
        for topic, side, entity in orphans:
            ...
            ctx.publish_period_ms = _pp_for(topic)
